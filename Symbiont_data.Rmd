...
sym_dom <- symbionts %>%
  ...
  dplyr::select(colony, days, vis, propD, score) # select columns of interest
...
sym_dom_graph %>% 
  ...
  ggplot(aes(x = days, y = propD, color = bleaching)) +
  geom_point() + 
  facet_wrap( ~ colony, nrow = 2) +
  theme(axis.text.x = element_text(angle = 90)) + # rotate x-axis labels 90 degrees
  labs(title = "Bleached", x = "Days", y = "Proportation of Clade D Symbionts")
